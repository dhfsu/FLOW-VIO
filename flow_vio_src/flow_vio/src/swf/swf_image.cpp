...
// 用来接收一帧（或双目两帧）图像，提取/跟踪特征并把结果放入后续处理队列 _img：左目图像，单目模式下就是当前图像。_img1：右目图像；单目模式下为空矩阵
...
void SWFOptimization::ImagePostprocess()
{
    ...
    if (f_manager.CheckParallax(image_count - 1))
        imag_marg_index = 0;
    else
        imag_marg_index = image_count - 2;
}
...
void SWFOptimization::OutliersRejection(int image_count)
{
    ...
    for (auto &it_per_id : f_manager.feature)
    {
        ...
        if (it_per_id.endFrame() > evaluate_frame_begin)
            ASSERT(imu_i >= evaluate_frame_begin);
        ASSERT(imu_i >= it_per_id.start_frame && imu_i <= it_per_id.endFrame());

        if (imu_i < evaluate_frame_begin || it_per_id.endFrame() <= evaluate_frame_begin)
            continue;

        if (is_cross)
            ASSERT(imu_i % SWF_SIZE_IN == 0);
        ...
        if (ave_err > OUTLIER_SHRESHOLD1)
        {
            ...
            if (it_per_id.start_frame < evaluate_frame_begin && evaluate_frame_begin > 0)
            {
                ...
                if (remain_size >= FEATURE_CONTINUE)
                    RegenerateFeatures(it_per_id, remain_size, new_features);
            }
        }
    }
