...
inline void SWFOptimization::AddFeatures(FeaturePerId &it_per_id, std::vector<VisualInertialBase *> &visual_inertial_bases, std::set<int> &factor_index_set)
{
    ...
    bool is_cross = (it_per_id.start_frame / SWF_SIZE_IN != (it_per_id.endFrame() - 1) / SWF_SIZE_IN) || (idepths_all[0].find(it_per_id.feature_id) != idepths_all[0].end() && last_marg_info->keep_block_addr_set.find(&idepths_all[0][it_per_id.feature_id]) != last_marg_info->keep_block_addr_set.end());
    ...
    for (auto &it_per_frame : it_per_id.feature_per_frame)
    {
        ...
        if (imu_j - 1 == (imu_i / SWF_SIZE_IN + 1) * SWF_SIZE_IN)
        {
            ...
            if (idepth_pointerj[0] == 0)
            {
                Eigen::Vector3d ptsInW =
                    Rs[imu_i] * (RIC[0] * (it_per_id.feature_per_frame[imu_i - it_per_id.start_frame].point /
                                           idepths_all[imu_i / SWF_SIZE_IN][it_per_id.feature_id]) +
                                 TIC[0]) +
                    Ps[imu_i];
        ...
        if (!is_cross)
        {
            if (factor_index_set.find(factor_index) == factor_index_set.end())
                continue;
            ...
            if (pos_pointer2indexs.find(parameters[0]) != pos_pointer2indexs.end())
                pindexi = pos_pointer2indexs[parameters[0]];
            else
                pindexi = pos_pointer2indexs.size();

            if (pos_pointer2indexs.find(parameters[1]) != pos_pointer2indexs.end())
                pindexj = pos_pointer2indexs[parameters[1]];
            else
                pindexj = pos_pointer2indexs.size();
...
void SWFOptimization::AddFactors(std::vector<VisualInertialBase *> &visual_inertial_bases, std::set<int> factor_index_set)
{
    ...
    for (int j = 1; j < image_count; j++)
    {
        ...
        std::vector<double *> parameters{para_pose[j - 1], para_speed_bias[j - 1],
                                         ...
                                         para_global_pos};
        if (ESTIMATE_TD)
            parameters.push_back(&td);
        if (ESTIMATE_EXTRINSIC)
            parameters.push_back(para_extrinsic);
        if (ESTIMATE_ACC_SCALE)
            parameters.push_back(acc_scale.data());
        if (ESTIMATE_GYR_SCALE)
            parameters.push_back(gyr_scale.data());
...
void SWFOptimization::UpdataGlobalFactors()
{
    ...
    if (old_max_factor_num == 0)
        visual_inertial_bases_global[0]->AddLastMargeInfo(last_marg_info);
    LOG_OUT << "add new\r\n";
}
...
void SWFOptimization::OptimizationOrMarginalization(int mode)
{
    ...
    if (factor_index_set.size())
        AddFactors(visual_inertial_bases2, factor_index_set);

    LEAK_NUM = old_leak_num;
    for (int factor_index = 0; factor_index < (int)visual_inertial_bases.size(); factor_index++)
    {
        if (visual_inertial_bases[factor_index]->imu_map_factors.size() == 0)
            continue;
    ...
    if (visual_inertial_bases[0]->last_marg_info == 0)
        visual_inertial_bases[0]->AddLastMargeInfo(last_marg_info);

    for (int factor_index = 0; factor_index < (int)visual_inertial_bases.size(); factor_index++)
    {
        if (visual_inertial_bases[factor_index]->imu_map_factors.size() == 0)
            continue;
        if (factor_index != 0 && mode == MargeMode)
            continue;
        ...
        if (factor_index != 0)
            ...
            visual_inertial_bases[factor_index]->prev_vibase = 0;
        ...
        if (factor_index != image_count / SWF_SIZE_IN && visual_inertial_bases[factor_index + 1] && visual_inertial_bases[factor_index + 1]->imu_map_factors.size())
            ...
            visual_inertial_bases[factor_index]->next_vibase = 0;

        visual_inertial_bases[factor_index]->FactorUpdates();

        solver_info.addResidualBlockInfo(new SolverResidualBlockInfo(
            new VisualInertialFactor(visual_inertial_bases[factor_index]), 0, visual_inertial_bases[factor_index]->outside_pointer_raw));
    }
    if (mode == MargeMode)
    {
    ...
    for (int factor_index = 0; factor_index < (int)visual_inertial_bases.size(); factor_index++)
    {
        if (visual_inertial_bases_global[factor_index])
            visual_inertial_bases_global[factor_index]->FactorUpdates();
    }
    ...
    {
        if (visual_inertial_bases_global[i] && !visual_inertial_bases_global[i + 1])
            visual_inertial_bases_global[i]->next_vibase = 0;
    }
    ...
    for (int i = 0; i < image_count / SWF_SIZE_IN + 1; i++)
    {
        if (visual_inertial_bases2[i])
            delete visual_inertial_bases2[i];
    }
