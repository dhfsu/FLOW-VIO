        ...
        if (it->solve_flag == 2)
        {
            ...
            for (int i = 0; i < (int)visual_inertial_bases_global.size(); i++)
            {
                if (visual_inertial_bases_global[i])
                    visual_inertial_bases_global[i]->RemoveFeature(it->feature_id);
            }
            it->RemoveIdepth();
...
void FeatureManager::removeFront(int image_index, Eigen::Vector3d P0, Eigen::Matrix3d R0, Eigen::Vector3d P1, Eigen::Matrix3d R1, Eigen::Vector3d tic0, Eigen::Matrix3d ric0)
{
    ...
    for (auto it = feature.begin(), it_next = feature.begin(); it != feature.end(); it = it_next)
    {
        ...
        if (marg_index % SWF_SIZE_IN == 0 && idepths_all[marg_index / SWF_SIZE_IN].find(it->feature_id) != idepths_all[marg_index / SWF_SIZE_IN].end() && idepths_all[marg_index / SWF_SIZE_IN][it->feature_id] && it->valid)
        {
